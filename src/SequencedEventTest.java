...
public class SequencedEventTest extends JFrame implements ActionListener {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            if (e != null && e.getClass().getName().equalsIgnoreCase("SequencedEvent")) {
                throw new RuntimeException("Test case failed. The last sequenced event is not yet dispatched.");
            }
    ...
    public void actionPerformed(ActionEvent e) {
        if(e.getSource() == spamMeButton) {
            AWTEvent eventOne = getSequencedEvent(this);
            AWTEvent eventFour = getSequencedEvent(this);
            ThreadGroup tg = new ThreadGroup("TestThreadGroup" );
            CountDownLatch latch = new CountDownLatch(1);
            Thread t = new Thread(tg, () -> {
                context = SunToolkit.createNewAppContext();
                AWTEvent eventTwo = getSequencedEvent(context);
                AWTEvent eventThree = getSequencedEvent(context);

                SunToolkit.insertTargetMapping(eventThree, context);
                SunToolkit.insertTargetMapping(eventTwo, context);
                AppContext ac2 = SunToolkit.targetToAppContext(eventTwo);
                AppContext ac3 = SunToolkit.targetToAppContext(eventThree);
                SunToolkit.targetToAppContext(eventThree);
                SunToolkit.postEvent(context, eventThree);
                SunToolkit.postEvent(context, new ActionEvent(context, 0, null));
                SunToolkit.postEvent(context, new ActionEvent(context, 1, null));
                SunToolkit.postEvent(context, eventTwo);
    ...
    private AppContext createAppContext (ThreadGroup tg) {
        try {
            Class<?> appContextClass = Class.forName("sun.awt.AppContext");
            Method getAppContextMethod = appContextClass.getMethod("getAppContext");
            Constructor<?> appContextConstructor = appContextClass.getConstructor(AppContext.class);
            appContextConstructor.setAccessible(true);

            return (AppContext)appContextConstructor.newInstance(tg);
        } catch (Throwable e) {
            throw new RuntimeException("Unable to instantiate AppContext", e);
        }
    }

    private AWTEvent getSequencedEvent(Object source)
    {
        AWTEvent wrapMe = new AWTEvent(source, AWTEvent.RESERVED_ID_MAX) {};
